...
class estarenergy extends eqLogic {
  ...
  */
  ...
  protected function fetchModuleDayData($login, $password, $stationId, $date = null) {
    ...
    if ($this->shouldRefreshModuleToken($binary)) {
      log::add('estarenergy', 'debug', __('Token module invalide détecté, rafraîchissement en cours...', __FILE__));
      $token = $this->retrieveToken($login, $password, $cookieFile, true);
      if ($token === null) {
        return null;
      }
  ...
  protected function queryModuleDayBinary($token, $cookieFile, $stationId, $date) {
    if ($this->isEmptyToken($token)) {
      log::add('estarenergy', 'debug', __('Token vide ou manquant pour l’appel down_module_day_data', __FILE__));
      return null;
    }
  ...
  protected function decodeModuleDayBinary($binary) {
    ...
    $embeddedJson = $this->extractEmbeddedJson($binary);
    if ($embeddedJson !== null) {
      $this->logJsonModuleDayError($embeddedJson);
      return null;
    }
  ...
  protected function extractEmbeddedJson($payload) {
    if (!is_string($payload) || $payload === '') {
      return null;
    }

    $printable = preg_replace('/[\x00-\x1F]+/', ' ', $payload);
    if ($printable === null) {
      $printable = $payload;
    }

    if (preg_match('/\{[^\{\}]*\}/s', $printable, $matches)) {
  ...
  protected function shouldRefreshModuleToken($binary) {
    $jsonPayload = $this->extractModuleDayJson($binary);
    if ($jsonPayload === null) {
      return false;
    }
    ...
    if ($this->isModuleDayTokenError($decoded)) {
      $this->logJsonModuleDayError($jsonPayload);
      return true;
    }
  ...
  protected function extractModuleDayJson($binary) {
    if (!is_string($binary) || $binary === '') {
      return null;
    }
    ...
    return $this->extractEmbeddedJson($binary);
  }

  protected function isModuleDayTokenError(array $decoded) {
    ...
    if (isset($decoded['message'])) {
      ...
      return strpos($message, 'token error') !== false || strpos($message, 're-login') !== false;
    }
